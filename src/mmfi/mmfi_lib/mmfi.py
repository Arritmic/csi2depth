...
class MMFi_Dataset(Dataset):
    ...
    def read_dir(self, dir):
        ...
        if mod in ['infra1', 'infra2', 'rgb']:  # rgb, infra1 and infra2 are 2D keypoints
            ...
            for img in sorted(glob.glob(os.path.join(dir, "frame*.png"))):
                ...
                depth_image_normalized = cv2.normalize(_cv_img, None, 0, 255, cv2.NORM_MINMAX)

                # Convert to uint8
                depth_image_normalized = depth_image_normalized.astype(np.uint8)

                data.append(depth_image_normalized)
            data = np.array(data)
        elif mod == 'lidar':
    ...
    def read_frame(self, frame):
        ...
        if mod in ['infra1', 'infra2', 'rgb']:
            ...
            depth_image_normalized = cv2.normalize(data, None, 0, 255, cv2.NORM_MINMAX)
            depth_image_normalized = depth_image_normalized.astype(np.uint8)

            data = depth_image_normalized
